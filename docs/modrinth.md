...
Wynncraft runs no voice server, so Simple Voice Chat alone does nothing there. WynnVoice is a
...
WynnVoice does not replace Simple Voice Chat; you install both. Simple Voice Chat's own
volume, push-to-talk and mute settings all apply. WynnVoice is a separate project: please
...
- **Audience.** You choose who may hear you with `/wynnvoice tier`: your **party** (default),
  your **friends and guild**, or **everyone** nearby. Party members hear each other anywhere.
- **Moderation.** `/wynnvoice block <player>` silences someone permanently in both directions.
  `/wynnvoice report <player> [reason]` reports someone you heard in the last two minutes.
  `/wynnvoice enable` and `/wynnvoice disable` turn the whole thing on and off.
...
- **Audio.** Your last two minutes of speech are held in the relay's memory only. They are
  ...
  audio and the reporter's own last two minutes — is sent to the WynnVoice maintainers' private
  Discord channel for review. Reports go to the WynnVoice maintainers, not to Wynncraft staff.
- **Bans** are for the WynnVoice relay as a whole and are enforced by the relay, not by the mod.

### Source

WynnVoice is free software under the LGPL-3.0-only license. The relay is open source too and
